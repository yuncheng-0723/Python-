...
# 顏色.
randomColor  = random.sample(range(0,255), 20)
print('random',randomColor )
color_block         = (0,0,0)
...
def brickNew():
    ...
    if (lines <= 3):
        # 消除連線數量累加.
        lines_number =  lines_number + lines *100
        # 修改連線數量.
        #modifyLabel(linesNumber, fontLinesNumber)
        # 1:清除磚塊.
        game_mode = 1
    elif (lines == 4):    
        lines_number =  lines_number + lines *200
        game_mode = 1
    # 初始方塊位置.
    container_x = 3
    container_y = -3
...
for y in range(20):
    for x in range(10):
        bricks_list[x][y] = Box(pygame, canvas, "brick_x_" + str(x) + "_y_" + str(y), [ 0, 0, 26, 26], (randomColor[random.randint(0,19)], randomColor[random.randint(0,19)], randomColor[random.randint(0,19)]))

# 將繪圖方塊放入陣列.
for y in range(4):
    for x in range(4):
        bricks_next_object[x][y] = Box(pygame, canvas, "brick_next_x_" + str(x) + "_y_" + str(y), [ 0, 0, 26, 26], (randomColor[random.randint(0,19)], randomColor[random.randint(0,19)], randomColor[random.randint(0,19)]))

# 背景區塊.
background = Box(pygame, canvas, "background", [ 278, 18, 282, 562], (155, 155, 155))

# 背景區塊.
background_bricks_next = Box(pygame, canvas, "background_bricks_next", [ 590, 50, 114, 114], (randomColor[random.randint(0,19)], randomColor[random.randint(0,19)], randomColor[random.randint(0,19)]))
...
while running:
    ...
    # 顯示訊息.
    showFont( "下次出現方塊", 588, 16, color_gray)
    showFont( "Score", 588, 190, color_gray)
    showFont( str(int(lines_number)), 588, 220, color_gray)
